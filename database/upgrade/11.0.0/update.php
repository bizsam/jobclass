...
try {
	...
	Schema::table('users', function (Blueprint $table) {
		...
		foreach ($indexes as $index) {
			$keyExists = DB::select(DB::raw('SHOW KEYS FROM ' . $tableName . ' WHERE Key_name="' . $index . '"'));
			if ($keyExists) {
		...
		foreach ($indexes as $index) {
			...
			$keyExists = DB::select(DB::raw('SHOW KEYS FROM ' . $tableName . ' WHERE Key_name="' . $indexName . '"'));
			if (!$keyExists) {
	...
	if (!Schema::hasColumn('posts', 'auth_field')) {
		Schema::table('posts', function (Blueprint $table) {
			$table->enum('auth_field', ['email', 'phone'])->nullable()->default('email')->after('application_url');
		});
	}
