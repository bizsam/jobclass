	...
	public function make($table, $field, Request $request)
	{
		...
		if (empty($info)) {
			...
			if (isset($info[0]) && isset($info[0]->DATA_TYPE)) {
				if ($table != 'settings' && $info[0]->DATA_TYPE != 'tinyint' && !str_ends_with($field, '_at')) {
					return response()->json($result, 200, [], JSON_UNESCAPED_UNICODE);
				}
				if ($table == 'settings' && $info[0]->DATA_TYPE != 'text' && $field == 'value') {
					return response()->json($result, 200, [], JSON_UNESCAPED_UNICODE);
				}
			} else {
				return response()->json($result, 200, [], JSON_UNESCAPED_UNICODE);
		...
		if (count($modelFiles) > 0) {
			foreach ($modelFiles as $filePath) {
				...
				if (
					!str_contains(strtolower($filename), '.php')
					|| str_contains(strtolower($modelName), 'base')
				) {
					continue;
				}
		...
		if ($table == 'countries' && $field == 'active') {
			...
			if (str_ends_with($field, '_at')) {
				$item->{$field} = (empty($item->{$field})) ? now() : null;
			} else {
				$item->{$field} = ($item->{$field} != 1) ? 1 : 0;
			}
			$item->save();
