...
trait TitleTrait
{
	...
	public function getHtmlTitle(?array $preSearch = [], ?array $sidebar = []): string
	{
		...
		// Company
		if (!empty(data_get($preSearch, 'company'))) {
			$htmlTitle .= ' ' . t('among') . ' ';
			$htmlTitle .= '<a rel="nofollow" class="jobs-s-tag" href="' . UrlGen::searchWithoutQuery() . '">';
			$htmlTitle .= data_get($preSearch, 'company.name');
			$htmlTitle .= '</a>';
		}
		...
		// Date
		$postedDate = request()->filled('postedDate') ? request()->get('postedDate') : null;
		$postedDateLabel = data_get($sidebar, 'periodsList.' . $postedDate);
		...
		// Job Type
		$requestedJobTypes = request()->filled('type') ? request()->get('type') : [];
	...
	public function getBreadcrumb(?array $preSearch = []): array
	{
		...
		if (!empty(data_get($preSearch, 'city'))) {
			...
			$bcTab[] = collect([
				'name'     => (!empty(data_get($preSearch, 'city')) ? t('All jobs') . ' ' . $title : data_get($preSearch, 'city.name')),
		...
		if (!empty(data_get($preSearch, 'admin'))) {
			...
			$bcTab[] = collect([
				'name'     => (!empty(data_get($preSearch, 'cat')) ? t('All jobs') . ' ' . $title : data_get($preSearch, 'admin.name')),
				'url'      => $searchUrl,
				'position' => (!empty(data_get($preSearch, 'cat')) ? 5 : 3),
				'location' => true,
			]);
		...
		if (!empty(data_get($preSearch, 'company'))) {
			$bcTab[] = collect([
				'name'     => data_get($preSearch, 'company.name'),
				'url'      => UrlGen::company(null, data_get($preSearch, 'company.id')),
				'position' => (!empty(data_get($preSearch, 'cat')) ? 5 : 3),
				'location' => true,
			]);
		}
